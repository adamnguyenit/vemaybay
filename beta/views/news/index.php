<?php
use app\assets\NewsIndexAsset;
use yii\helpers\Url;

NewsIndexAsset::register($this);
$this->title = 'Vé máy bay Hải Phi Yến | Tin tức';
?>
<div class="container">
    <div class="row">
        <div id="left-side" class="col-md-9">
            <div class="row">
                <div class="news bg-white">
                    <img class="pull-left hidden-xs" src="http://placehold.it/600x400">
                    <h3><a class="color-red" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</a></h3>
                    <p class="text-muted"><span class="fa fa-calendar"></span> Thứ 2, 24/04/2016</p>
                    <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Aenean commodo ligula eget dolor.</p>
                    <a class="btn btn-primary pull-right" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>" role="button">Đọc tiếp</a>
                </div>
            </div>
            <div class="row">
                <div class="news bg-white">
                    <img class="pull-left hidden-xs" src="http://placehold.it/600x400">
                    <h3><a class="color-red" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</a></h3>
                    <p class="text-muted"><span class="fa fa-calendar"></span> Thứ 2, 24/04/2016</p>
                    <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Aenean commodo ligula eget dolor.</p>
                    <a class="btn btn-primary pull-right" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>" role="button">Đọc tiếp</a>
                </div>
            </div>
            <div class="row">
                <div class="news bg-white">
                    <img class="pull-left hidden-xs" src="http://placehold.it/600x400">
                    <h3><a class="color-red" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</a></h3>
                    <p class="text-muted"><span class="fa fa-calendar"></span> Thứ 2, 24/04/2016</p>
                    <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Aenean commodo ligula eget dolor.</p>
                    <a class="btn btn-primary pull-right" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>" role="button">Đọc tiếp</a>
                </div>
            </div>
            <div class="row">
                <div class="news bg-white">
                    <img class="pull-left hidden-xs" src="http://placehold.it/600x400">
                    <h3><a class="color-red" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</a></h3>
                    <p class="text-muted"><span class="fa fa-calendar"></span> Thứ 2, 24/04/2016</p>
                    <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Aenean commodo ligula eget dolor.</p>
                    <a class="btn btn-primary pull-right" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>" role="button">Đọc tiếp</a>
                </div>
            </div>
            <div class="row">
                <div class="news bg-white">
                    <img class="pull-left hidden-xs" src="http://placehold.it/600x400">
                    <h3><a class="color-red" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</a></h3>
                    <p class="text-muted"><span class="fa fa-calendar"></span> Thứ 2, 24/04/2016</p>
                    <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Aenean commodo ligula eget dolor.</p>
                    <a class="btn btn-primary pull-right" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>" role="button">Đọc tiếp</a>
                </div>
            </div>
            <div class="row">
                <div class="news bg-white">
                    <img class="pull-left hidden-xs" src="http://placehold.it/600x400">
                    <h3><a class="color-red" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</a></h3>
                    <p class="text-muted"><span class="fa fa-calendar"></span> Thứ 2, 24/04/2016</p>
                    <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Aenean commodo ligula eget dolor.</p>
                    <a class="btn btn-primary pull-right" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>" role="button">Đọc tiếp</a>
                </div>
            </div>
            <div class="row">
                <div class="news bg-white">
                    <img class="pull-left hidden-xs" src="http://placehold.it/600x400">
                    <h3><a class="color-red" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</a></h3>
                    <p class="text-muted"><span class="fa fa-calendar"></span> Thứ 2, 24/04/2016</p>
                    <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Aenean commodo ligula eget dolor.</p>
                    <a class="btn btn-primary pull-right" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>" role="button">Đọc tiếp</a>
                </div>
            </div>
        </div>
        <div id="right-side" class="col-md-3 hidden-sm hidden-xs">
            <div class="row" style="margin-left: -10px">
                <div id="popular-box">
                    <div class="popular-news bg-white">
                        <h3><a href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</a></h3>
                        <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Aenean commodo ligula eget dolor.</p>
                        <a class="btn btn-sm btn-primary pull-right" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>" role="button">Đọc tiếp</a>
                    </div>
                    <div class="popular-news bg-white">
                        <h3><a href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</a></h3>
                        <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Aenean commodo ligula eget dolor.</p>
                        <a class="btn btn-sm btn-primary pull-right" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>" role="button">Đọc tiếp</a>
                    </div>
                    <div class="popular-news bg-white">
                        <h3><a href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</a></h3>
                        <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Aenean commodo ligula eget dolor.</p>
                        <a class="btn btn-sm btn-primary pull-right" href="<?= Url::toRoute(['news/info', 'alias' => 'lorem-ipsum-dolor-sit-amet-consectetuer-adipiscing-elit']) ?>" role="button">Đọc tiếp</a>
                    </div>
                </div>
                <div id="panel-box">
                    <div class="panel">
                        <img src="http://placehold.it/600x900">
                    </div>
                    <div class="panel">
                        <img src="http://placehold.it/600x900">
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
